...
| #5 detección N/A | ✅ Implementado | Dos bugs: (a) clave errónea `m.confidence_level` → `m.detection_confidence_level \|\| m.confidence_level` en `analysis-core.js:20373` (`generarHTMLReporteParaBatch`) y en `project-manager.js:2837` (CSV); (b) `detection_method` sin fallback → añadida cadena `m.detection_method \|\| m.detectionMethod \|\| m.detection_mode`. Cache-bust `analysis-core.js?v=20260912b`, `project-manager.js?v=20260912a` |
