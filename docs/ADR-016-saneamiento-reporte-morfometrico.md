...
| #5 detección N/A | ✅ **Resuelto por ADR-019** | La causa no era sólo la cabecera: el objeto IA nunca llevaba los campos, y la cadena estaba cortada en 5 puntos (clave de nivel divergente, gate `_samSegmented` sobre la confianza, objeto IA sin sellar, `params_used` descartado, merge-drop en la fusión Python). ADR-019 introduce el escritor único `MaoDeteccion.aplicarProcedencia` y la sección «I. Detección del Objeto» en panel/tabla/CSV/PDF |
...
| #11 distancia asimetría contextualización | ✅ Implementado | `tabla-metricas-completa.js:generarSeccionSimetria` — se añade `distPct = distanciaAsimetria / ejeMayorRef × 100` y se muestra junto al valor absoluto: «X.XX mm (Y.Y% del eje mayor)». La descripción del campo cambia de «Desplazamiento del eje de simetría» (inexacto) a «Residuo Hausdorff promedio respecto al radio medio del contorno» (fiel a la fórmula). El revisor puede ahora reconciliar el valor mm con la clasificación de simetría usando la escala del objeto. Cache-bust `analysis-core.js?v=20260912e` |
