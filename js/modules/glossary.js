...
export const CONVENCIONES = [
  {
    ...
    termino: 'IA',
    expansion: 'Identificación Automatizada',
    nota: 'En MAO Plus la sigla **IA** significa **Identificación Automatizada** — el modo de detección en que el sistema aísla la pieza sin que el operador trace ni encuadre. **No** se usa como abreviatura de «inteligencia artificial». La distinción importa porque el nombre designa QUÉ HACE el modo (identificar automáticamente), no con qué técnica lo hace: ese dato es independiente y se consigna pieza a pieza en `ia_segmentador`, que registra el motor concreto empleado. Un mismo análisis en modo IA puede haberse resuelto con el núcleo OpenCV, con un segmentador neuronal como prior, o con ambos. Leer la sigla como «inteligencia artificial» induce a atribuir a un modelo estadístico resultados que a menudo produjo la umbralización clásica, y a desconfiar de mediciones que son tan deterministas como las del modo automático.',
  },
  {
    id: 'sigla_aia', tipo: 'sigla',
    termino: 'AIA',
    expansion: 'sigla retirada — decir IA (Identificación Automatizada)',
    nota: '«AIA» aparecía como segunda sigla del mismo módulo, **nunca desarrollada en ningún sitio**: la guía la usaba para rotular «Módulo MAO IA — Detección AIA» y un botón, y `collection.js` la muestra al usuario en dos avisos («Actualizando métricas del análisis AIA…»). Dos siglas sin expandir para una misma cosa duplican la ambigüedad que la convención de la sigla **IA** vino a cerrar. **Convención: la sigla es IA; AIA no se usa en texto nuevo.** Los identificadores internos que la conservan (`_isAIA`, `[AIA-enrich]`) son legibles solo para quien edita el código y pueden quedarse hasta que se toquen por otra razón; los textos que ve el usuario, no.',
  },
  {
...
export const TERMINOS = [
  ...
  // ═══════════════════════════════════════════════════════════════════════
  {
    ...
    rango: 'automático | manual de área | manual por componente | IA — Identificación Automatizada (segmentación asistida)',
    definicion: 'Modo por el que se aisló la pieza del fondo. La sigla **IA** designa aquí **Identificación Automatizada**, no «inteligencia artificial» (ver las convenciones de nomenclatura). Los cuatro modos comparten el mismo núcleo de segmentación (OpenCV: Z-scan + CLAHE + GrabCut + watershed); se diferencian en el prior que recibe ese núcleo — ninguno, un rectángulo, un clic, o una máscara neuronal.',
    interpretacion: 'Condiciona la comparabilidad entre piezas: un lote medido con priors manuales heterogéneos no es directamente comparable con uno automático. Declararlo es requisito para reproducir la medición.',
    fuente: 'js/modules/detection-section.js::metodoLegible', nivel: null, ref: null,
    nota: 'ADR-012: el núcleo es único; el modo es un prior, no una reimplementación.',
  },
  {
  ...
  },
  {
    ...
    rango: 'nombre del motor de segmentación empleado (p. ej. MobileSAM, GrabCut)',
    definicion: 'Motor concreto que aportó el prior de segmentación en modo IA — Identificación Automatizada. Es el campo que dice CON QUÉ se resolvió el aislamiento de la pieza, dato que la sigla del modo deliberadamente no prejuzga.',
    interpretacion: 'Parte de la trazabilidad del dato, y la única fuente fiable sobre si intervino un modelo entrenado: dos versiones del mismo segmentador pueden producir contornos distintos sobre la misma imagen. Vacío cuando el modo no recurrió a ningún motor externo.',
    fuente: 'python/modules/mao_ia_analyzer.py', nivel: null, ref: null, nota: null,
  },
  {
    clave: 'ia_threshold_method', ambito: 'objeto', categoria: 'deteccion',
    csv: ['Detección', 'Umbralización (modo IA)'], nombre: 'Umbralización (modo IA)', formula: null, unidad: 'texto',
    rango: 'auto | otsu | adaptativo | manual',
    definicion: 'Estrategia de umbral aplicada dentro del modo IA — Identificación Automatizada. El valor `auto` delega en el núcleo OpenCV canónico y solo después enriquece el resultado.',
    interpretacion: 'Con `auto` la pieza es comparable con las detectadas en modo automático, porque el contorno sale del mismo núcleo: el resultado es tan determinista como el de aquel, pese a lo que la sigla pueda sugerir a quien la lea como «inteligencia artificial».',
    fuente: 'python/modules/mao_ia_analyzer.py::detect_with_mao_ia', nivel: null, ref: null, nota: null,
  },
  {
    clave: 'ia_enriquecido', ambito: 'objeto', categoria: 'deteccion',
    csv: ['Detección', 'Enriquecido por IA'], nombre: 'Enriquecido por IA', formula: null, unidad: 'texto',
    rango: 'Sí | No',
    definicion: 'Indica si el modo IA — Identificación Automatizada aportó información adicional sobre el resultado del núcleo (tipología sugerida, refinamiento de máscara).',
    interpretacion: 'Distingue una pieza medida solo por el núcleo de una cuyo contorno fue además modificado por el segmentador. Importa al auditar de dónde viene una forma: con `Sí`, el contorno no es reproducible únicamente con los parámetros del núcleo.',
    fuente: 'python/modules/mao_ia_analyzer.py', nivel: null, ref: null, nota: null,
  },
  ...
  },
  {
    ...
    fuente: `${METRICS}::compute_metrics`, nivel: null, ref: null,
    nota: 'ADR-016 #1: en la ruta IA este campo puede llegar en píxeles; el conversor `MetricPresenter.conversorBBaMm` lo normaliza en el reporte.',
  },
  {
