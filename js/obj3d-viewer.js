    ...
    if (efaPanelEl) {
      ...
      if (!efa?.available) {
        ...
        efaPanelEl.innerHTML = [
          ...
          `<button class="obj3d-copy-btn" onclick="navigator.clipboard.writeText(${JSON.stringify(sigJson)}).then(()=>{this.textContent='¡Copiada!';setTimeout(()=>{this.textContent='Copiar Firma'},1500)})" title="Vector firma plano (nh×4): huella morfológica del objeto dentro de MAO. Convenio interno de MAO (ADR-021): NO intercambiable con Momocs ni pyefd">Copiar Firma</button>`,
          `<button class="obj3d-copy-btn" onclick="navigator.clipboard.writeText(${JSON.stringify(coeffJson)}).then(()=>{this.textContent='¡Copiado!';setTimeout(()=>{this.textContent='Copiar JSON'},1500)})" title="Coeficientes medios como matriz JSON para efa.compare(). Convenio interno de MAO (ADR-021): NO intercambiable con Momocs ni pyefd">Copiar JSON</button>`,
